...
| ZIP-level weather and market alerts                                 | Evidence lifecycle staged; notifications remain off         | Service-only `v_collision_zip_alert_candidates`; atomic three-day SPC refresh; historical proxy shows 4.89% follow-through versus 4.39% control; staged cases require owner/manager acknowledgement and expose audited exact 1–4 week shop/ZIP repair arrivals against a trailing 52-week baseline | Apply the lifecycle migration, name the organization-level notification owner, pre-register an economic threshold, and accrue prospective cases before authorizing notifications |
...
- Weather review evidence uses four non-overlapping seven-day windows after the UTC
  event date and the same shop/customer-ZIP's prior 364 days. Observed outcomes require
  four complete weeks and repair history spanning the prior 364 days; closure snapshots counts
  and repair-feed freshness in the case and immutable audit event. The database derives
  follow-through from the registered arrival-count threshold and rejects contradictory
  operator input. Notifications remain disabled and no production schema or data
  changed.
