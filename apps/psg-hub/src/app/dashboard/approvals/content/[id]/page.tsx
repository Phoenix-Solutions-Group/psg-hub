              ...
              <div key={comment.id} className="border-t border-border pt-3 first:border-t-0 first:pt-0">
                ...
                {comment.attachment && (
                  <div className="mt-2 rounded-md border border-border bg-muted/30 p-2 text-xs text-muted-foreground">
                    <p className="font-medium text-foreground">{comment.attachment.originalFilename}</p>
                    <p>
                      {formatBytes(comment.attachment.byteSize)} ·{" "}
                      {comment.attachment.screeningStatus === "passed_basic_screen"
