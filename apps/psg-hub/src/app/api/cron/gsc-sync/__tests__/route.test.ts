...
vi.mock("@/lib/google-oauth/gsc-sync", () => ({
  syncGscSnapshots: (...args: unknown[]) => syncMock(...args),
}));
...
function req(auth?: string): Request {
  return new Request("http://localhost/api/cron/gsc-sync", {
...
describe("cron/gsc-sync gate", () => {
  ...
  it("503 gsc_not_configured when the redirect (shared Google creds) is missing — NO dev token gate", async () => {
    vi.stubEnv("GOOGLE_ANALYTICS_OAUTH_REDIRECT_URI", "");
    const res = await GET(req("Bearer test-secret"));
    expect(res.status).toBe(503);
    expect(await res.json()).toEqual({ error: "gsc_not_configured" });
    expect(syncMock).not.toHaveBeenCalled();
  });
});

describe("cron/gsc-sync happy path", () => {
