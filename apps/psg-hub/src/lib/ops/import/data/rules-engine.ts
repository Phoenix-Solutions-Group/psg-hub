// PSG-132 — Import rules engine: ampersand handling, fleet/commercial detection,
// deduplication.
//
// Ported verbatim from `Phoenix-Solutions-Group/import` @ main `a11133d`
// (`src/lib/processing/rules-engine.ts`) ahead of the PSG-50 decommission.
// `applyRules` partitions canonical-38 rows (./types `Row`) into clean vs error
// sets: decodes HTML-entity ampersands, flags fleet/commercial owners by keyword
// (and company-name-without-person heuristic), and de-dupes on name+address+phone
// keeping the row with the newer DeliveredDate.
