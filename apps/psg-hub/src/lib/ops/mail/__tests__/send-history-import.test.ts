...
// Stable, non-secret salt so hashes are deterministic in CI. Production sets
// MAIL_HASH_SALT; the importer threads this straight into the shared household.ts
// hasher (PSG-221), the same salt mail_suppression uses.
const TEST_SALT = "psg-mail-test-salt-v0";
...
describe("importSendBatch — reconciliation (AC1) on synthetic fixture", () => {
  ...
  it("collapses send_ref duplicates (idempotency, not an error)", () => {
    // Block #2 is the same recipient as #1 but with "123 Main Street" vs
    // "123 Main St" — USPS normalization (in the shared household.ts hasher)
    // collapses them to one send_ref, proving re-import idempotency.
  ...
  it("persists NO raw PII — only salted hashes + shop key (AC4)", () => {
    for (const r of records) {
      expect(r.recipient_hash).toMatch(/^rc_[0-9a-f]{64}$/);
      expect(r.household_key).toMatch(/^hh_[0-9a-f]{64}$/);
