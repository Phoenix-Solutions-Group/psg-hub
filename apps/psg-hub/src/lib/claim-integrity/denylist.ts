...
/**
 * Testimonial-shaped quote detection for SOP Section 9. The writer may quote a
 * customer only when that exact quote is already in the approved review list.
 * This scan deliberately targets quotes with review/customer framing, not every
 * quoted phrase, so normal copy like "call us today" is not treated as a review.
 */
const TESTIMONIAL_PATTERNS: readonly RegExp[] = [
  /["“]([^"”]{12,260})["”]\s*(?:[—-]\s*)?(?:[A-Z][A-Za-z .'-]{1,50}|a\s+(?:customer|client|reviewer)|happy\s+customer|verified\s+customer|google\s+reviewer)\b/gi,
  /\b(?:customer|client|reviewer|google\s+review)\s+(?:said|says|shared|wrote|noted),?\s*["“]([^"”]{12,260})["”]/gi,
  /\b(?:customers?|clients?)\s+(?:say|tell us|love)\b[^"“\n]{0,80}["“]([^"”]{12,260})["”]/gi,
  /^>\s*["“]?([^"\n”]{12,260})["”]?(?:\s*[—-]\s*(?:[A-Z][A-Za-z .'-]{1,50}|a\s+(?:customer|client|reviewer)|google\s+reviewer))?\s*$/gim,
];

function normalizeReviewQuote(value: string): string {
  return value
    .trim()
    .replace(/^["“]+|["”]+$/g, "")
    .replace(/\s+/g, " ")
...
function isApprovedReviewQuote(candidate: string, approvedQuotes: readonly ReviewQuote[]): boolean {
  const normalizedCandidate = normalizeReviewQuote(candidate);
  return approvedQuotes.some((review) => {
    const normalizedApproved = normalizeReviewQuote(review.quote);
    return (
      normalizedApproved === normalizedCandidate ||
      normalizedApproved.includes(normalizedCandidate) ||
      normalizedCandidate.includes(normalizedApproved)
    );
  });
}

/** Scan for generated or unlogged testimonial/review quotes. */
export function scanTestimonials(
  text: string,
  approvedQuotes: readonly ReviewQuote[] = [],
): Violation[] {
  const violations: Violation[] = [];
  ...
  for (const pattern of TESTIMONIAL_PATTERNS) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const key = normalizeReviewQuote(quote);
      if (seen.has(key)) continue;
      seen.add(key);

      if (!isApprovedReviewQuote(quote, approvedQuotes)) {
        violations.push({
          code: "unapproved_testimonial",
          message:
            "Copy includes a testimonial/review-shaped quote that is not in the approved review list. Customer reviews must come from a real logged source; the writer must not invent one (SOP Section 9).",
          evidence: quote,
        });
      }
    }
...
export function scanDenylist(
  ...
  return [
    ...
    // SOP Section 9 — reviews/testimonials must be logged real quotes, never
    // self-generated by the writer.
    ...scanTestimonials(text, approvedQuotes),
