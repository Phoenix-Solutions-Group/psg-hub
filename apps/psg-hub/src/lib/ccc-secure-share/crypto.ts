...
// CCC Secure Share credential encryption. Cloned from src/lib/google-ads/crypto.ts
// (the recorded app-level AES-256-GCM deviation): versioned env keys, IV-prefixed
// + auth-tagged ciphertext. The credential SHAPE is abstract (Phase 0 / PSG-251
// confirms OAuth-refresh-token vs API-key vs cert) — this module encrypts an
// opaque UTF-8 credential string regardless of which scheme lands.
...
function loadKeyMap(): {
  ...
  loadFrom("CCC_ENCRYPTION_KEY", 1);
  for (let v = 2; v <= 10; v++) {
    loadFrom(`CCC_ENCRYPTION_KEY_V${v}`, v);
  }
  ...
  if (versions.length === 0) {
    throw new Error("No CCC_ENCRYPTION_KEY set");
  }
