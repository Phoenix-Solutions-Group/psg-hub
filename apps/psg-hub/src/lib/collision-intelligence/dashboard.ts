...
export type CollisionDashboardScope = {
 ...
 * both required.
 */
export async function getCollisionDashboard(
  shopId: string,
  scope?: CollisionDashboardScope,
) {
  const service = createServiceClient();
  const sourceShopIds = [...new Set(scope?.sourceShopIds ?? [shopId])];
  const primaryShopId = scope?.primaryShopId ?? shopId;
  if (!sourceShopIds.length || !sourceShopIds.includes(primaryShopId)) {
    throw new Error("Collision dashboard scope is invalid");
  }
  ...
  const merged = mergeCollisionScopeRows(
    {
      weekly: (weekly.data ?? []) as CollisionWeeklyRow[],
      weather: (weather.data ?? []) as CollisionWeatherRow[],
      forecast: (forecast.data ?? []) as CollisionForecastRow[],
      crashes: (crashes.data ?? []) as CollisionCrashRow[],
      alerts: (alerts.data ?? []) as CollisionAlertRow[],
      insurers: (insurers.data ?? []) as CollisionInsurerRow[],
      customerZips: (customerZips.data ?? []) as CollisionZipRow[],
      vehicles: (vehicles.data ?? []) as CollisionVehicleRow[],
      quality: (quality.data ?? []) as CollisionQualityRow[],
      seasonality: (seasonality.data ?? []) as CollisionSeasonalityRow[],
    },
    scope?.displayName ??
      (weekly.data?.[0]?.company_name as string | undefined) ??
      "Repair demand",
  );

  let nationalCrashRows: CollisionNationalCrashRow[] = [];
  let nationalCrashSourceRows: CollisionNationalCrashSourceRow[] = [];
  const topCustomerState = merged.customerZips[0]?.customer_state;

  if (!merged.crashes.length && topCustomerState) {
  ...
  return scope ? { ...dashboard, companyName: scope.displayName } : dashboard;
}
