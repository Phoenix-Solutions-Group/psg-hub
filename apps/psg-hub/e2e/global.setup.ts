...
async function seedPrimaryLocation(shopId: string, shopName: string): Promise<string> {
  const slug = "primary";
  const { data, error } = await admin
    .from("locations")
    .upsert(
      { shop_id: shopId, name: shopName, slug, is_primary: true },
      { onConflict: "shop_id,slug" }
    )
    .select("id")
    .single();
  if (error || !data) throw new Error(`[e2e] location seed failed: ${error?.message}`);
  return data.id as string;
}

async function seedSubscription(
  shopId: string,
  tier: "essentials" | "growth" | "performance"
): Promise<void> {
  const { error } = await admin.from("subscriptions").upsert(
    {
      shop_id: shopId,
      stripe_customer_id: `cus_e2e_${shopId}`,
      stripe_subscription_id: `sub_e2e_${shopId}`,
      tier,
      status: "active",
      current_period_end: `${SNAPSHOT_END_DATE}T23:59:59Z`,
    },
    { onConflict: "shop_id" }
  );
  if (error) throw new Error(`[e2e] subscription seed failed: ${error.message}`);
}
...
async function seedBsmContentApprovalReview(
  ...
  if (itemErr) {
    throw new Error(`[e2e] BSM content review item seed failed: ${itemErr.message}`);
  }
...
async function seedPortalContentItem(
  shopId: string,
  locationId: string,
  opts: { itemId: string }
): Promise<void> {
  const { error } = await admin.from("content_items").upsert(
    {
      ...
      updated_at: SNAPSHOT_SYNCED_AT,
    },
    { onConflict: "id" }
  );
  if (error) throw new Error(`[e2e] content item seed failed: ${error.message}`);
}

async function seedPortalReview(
  shopId: string,
  locationId: string,
  ...
  const { error: reviewErr } = await admin.from("review_items").upsert(
    {
      ...
      platform: "google",
      rating: 5,
      text: "Riverside Collision kept me updated and finished the repair earlier than expected.",
      author: "Morgan Lee",
      reviewed_at: `${SNAPSHOT_END_DATE}T10:00:00Z`,
      external_review_id: "e2e-riverside-review-1",
      updated_at: `${SNAPSHOT_END_DATE}T10:00:00Z`,
    },
    { onConflict: "id" }
  );
  if (reviewErr) throw new Error(`[e2e] review item seed failed: ${reviewErr.message}`);

  const { error: sentimentErr } = await admin.from("review_sentiment").upsert(
    {
      review_item_id: opts.reviewId,
      shop_id: shopId,
      polarity: "positive",
      confidence: 0.92,
      themes: ["communication", "speed"],
      actionable_complaint: false,
      model_id: "e2e-fixture",
      prompt_version: "e2e",
      classified_updated_at: `${SNAPSHOT_END_DATE}T10:00:00Z`,
    },
    { onConflict: "review_item_id" }
  );
  if (sentimentErr) throw new Error(`[e2e] review sentiment seed failed: ${sentimentErr.message}`);
}
...
setup("seed fixtures + per-role storageState", async ({ browser }) => {
  ...
  await seedProfile(ownerId, "BSM Demo User");
  await ensureCustomerRole(ownerId);
  const ownerShopId = await seedShop(ownerId, OWNER.shopName, "owner");
  const ownerLocationId = await seedPrimaryLocation(ownerShopId, OWNER.shopName);
  await seedSubscription(ownerShopId, "performance");
  ...
  await seedPrimaryLocation(shopAId, MULTI.shopA);
  await seedPrimaryLocation(shopBId, MULTI.shopB);
  ...
  for (const name of MEGA.shopNames) {
    const shopId = await seedShop(megaId, name, "owner");
    await seedPrimaryLocation(shopId, name);
  }
  ...
  await seedProfile(opsStaffId, "BSM Demo Admin");
  ...
  await seedPortalContentItem(ownerShopId, ownerLocationId, {
    itemId: OWNER.contentItemId,
  });
  await seedPortalReview(ownerShopId, ownerLocationId, {
    reviewId: OWNER.reviewItemId,
  });
