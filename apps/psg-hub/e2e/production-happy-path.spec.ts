 ...
 *                          document through the selected test vendor and moves
 *                          the batch printing->historical.
 ...
 *
 * When LOB_API_KEY is available the print leg uses Lob test mode. Local recovered
 * workspaces usually do not have that secret, so the same browser gate uses the
 * in-house adapter to verify the production workflow without live vendor access.
 * The production_reprint_log audit row has no read API, so the audit assertion
 * reads it back with the same service-role client the seed uses.
 */
...
async function seedReleasedWarrantyTemplate(): Promise<void> {
  ...
  expect(usersError, usersError?.message).toBeNull();
  const opsUser = users.users.find((user) => user.email === OPS_STAFF.email);
  expect(opsUser?.id, "E2E ops auth user id").toBeTruthy();
  ...
  expect(profile?.id, "E2E ops profile id").toBeTruthy();
  ...
  const { error } = await admin.from("mail_template_approvals").upsert(
    {
      ...
      approved_by_name: BSM_DEMO_ADMIN.displayName,
      approved_at: now,
      released_by_profile_id: profile!.id,
      released_at: now,
...
test("production happy path: generate -> print (Lob test) -> historical -> reprint (audited)", async ({
  ...
  expect(gen.vendor).toBe(productionTestVendor());

  // The queued batch shows up in the /ops/production print queue.
  await page.goto("/ops/production");
  const batchRow = page.locator("tr").filter({ hasText: PROD_OPS.batchName }).filter({
    has: page.getByRole("button", { name: "Print batch" }),
  }).first();
  await expect(batchRow.getByText(PROD_OPS.batchName)).toBeVisible();
  ...
  expect(printed.vendor).toBe(productionTestVendor());
  ...
  // The production surface now shows the printed document under Recent documents,
  // with the vendor print id surfaced on the document row.
  await page.goto("/ops/production");
  const recentDocuments = page.locator("section").filter({
    has: page.getByRole("heading", { name: "Recent documents" }),
  });
  await expect(recentDocuments.getByText(PROD_OPS.batchName).first()).toBeVisible();
  await expect(recentDocuments.getByText(externalId)).toBeVisible();
  ...
  expect(reprinted.vendor).toBe(productionTestVendor());
