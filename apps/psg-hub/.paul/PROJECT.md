...
### Active (In Progress)
**v0.3 Customer Analytics 🚧 (created 2026-06-04, 0/3 phases).** Unified Google Ads + GA4 + GSC + SEMrush marketing surface, output as the automated PSG monthly client report (multi-LLM narrative + branded PDF). Phases: **9** Google Ads analytics surface (BSM Phase 5 data, no OAuth; + MSO cross-shop aggregate + switcher typeahead + LCP<2s gate) · **10** Unified data GA4+GSC+SEMrush (per-shop Google OAuth D68, refresh tokens pgsodium-encrypted; SEMrush account-level MCP) · **11** PSG report (narrative + monthly PDF spec'd to the existing hand-built client deliverable, template canon = `archive/local_reach-outputs/`). Cut from v0.3 → v0.3.5+: digital presence/listings + post-repair sentiment. **Next: `/paul:plan` Phase 9.** ── v0.2 Customer MVP ✅ COMPLETE 2026-06-04, promoted + LIVE on hub.psgweb.me (merged main `fc6e1cc`, dpl_Wyi5, tag `psg-hub-v0.2.0`); archived to MILESTONES.md + `milestones/v0.2.0-ROADMAP.md`.
