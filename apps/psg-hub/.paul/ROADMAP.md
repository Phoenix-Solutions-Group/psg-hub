...
|-------|------|-------|--------|-----------|
| 15 | Billing foundation + Stripe spine | 3 | 🚧 Planning | - |
...
<details>
<summary>v0.3.5 Presence + Sentiment — 2026-06-17 (2 phases, 10 plans)</summary>

| Phase | Name | Plans | Completed |
|-------|------|-------|-----------|
| 13 | GBP presence foundation + insights | 6/6 | 2026-06-16 (activation-pending tail) |
| 14 | Reviews + sentiment | 4/4 | 2026-06-17 (LIVE on prod; 385 Wallace reviews) |

Both verticals LIVE on prod. EXTEND-not-build held milestone-wide (0 new runtime deps). Full entry: `.paul/MILESTONES.md`; archive: `.paul/milestones/v0.3.5-ROADMAP.md`; tag `v0.3.5`. Carried-forward: rotate the chat-exposed GBP secret; sentiment prod backfill (cron); D3 7-day token pass-gate; systemic locations fleet backfill; 14-02b (reply/legal) + 14-03b (sentiment surface).

</details>

<details>
<summary>v0.3 Customer Analytics — 2026-06-13 (4 phases, 18 plans)</summary>
...
| 12 | PSG report — narrative + PDF | 7/7 | 2026-06-13 |
...
- [ ] 12-05 GA4-dimensional + real-performance expansion (ADDED 2026-06-11, operator decision after reviewing the live "Wallace Collision GA4 Monthly Analytics Report" Looker Studio deliverable). Adds, beyond the 12-01..12-04 date-totals base: GA4 secondary-dimension ingest + report sections — Top Traffic Drivers (by channel/source), Top Landing Pages (by landingPage), Device Breakdown (deviceCategory), New vs Returning (newVsReturning) — plus the two missing GA4 metrics bounce rate + avg session duration; AND real website-performance metrics from PROPER sources (Google CrUX / PageSpeed Insights + the GTMetrix API — operator has access), REPLACING the existing report's dubious GA4 "Performance Status" block (GA4 has no page-load/server-response metrics; "server response 14:49" is a mis-mapped duration — not ingested). **Requires RESEARCH first (hard gate — CrUX + PageSpeed + GTMetrix are new external APIs; GA4 dimensional ingest is a new data-model shape; prefer ultracode Workflow).** Activates on the SAME 12-04 infra (no new worker — incremental ingest + new secrets + redeploy). **CLOSES Phase 12 + milestone v0.3.**
  ...
  - Split into a **3-plan arc** (operator-confirmed 2026-06-11):
    ...
    - [x] **12-05c** Cron wiring + combined operator gate batch — **✅ LOOP CLOSED 2026-06-13 (closes Phase 12 + milestone v0.3)** (`12-05c-SUMMARY.md`; 3 tasks / 4 ACs). T1+T2 build-local (tsc 0 / eslint 0 / vitest 584 / build green): `getMonthlySnapshot` + `readMonthlyDimensions`/`readMonthlyPerformance` wired into the PRINT path only (closes the GAP; narrative binding untouched/eval-safe); two CRON_SECRET cron routes (`ga4-dims-sync` `0 2 1`, `perf-sync` `0 3 1`) prior-month-injected + GTMetrix pilot-scoped via `GTMETRIX_SHOP_IDS`; vercel.json 5→7 crons, report moved `0 0 1`→`0 5 1`. T3 operator gate **through Stage B**: both 12-05a/b migrations applied under PROTOCOL (advisor zero-delta, 6-value source CHECKs verified); `PAGESPEED_API_KEY`+`GTMETRIX_API_KEY`+`GTMETRIX_SHOP_IDS` set; `vercel --prod` → dpl_FEDz4AE6mWxrydVcJHQ9Gsb2h7kn READY (7 crons live, routes 401-gated); Wallace shops.url set. **Stage C live smoke SKIPPED → ACTIVATION-PENDING** (CRON_SECRET Vercel-Sensitive/un-pullable, operator declined rotation): the GA4-dims + perf ingests and their build-blind parsers (GA4 `totals[0]`, PSI `loadingExperience`, GTMetrix `/reports/{id}`) first run UNMONITORED on the July 1 crons — **section-correctness verification owed before/right-after July 1**. Base report live since 12-04; expansion degrades gracefully. Stage D merge→main + secret rotation = milestone-close hygiene.
...
## v0.3.5 — Presence + Sentiment (✅ COMPLETE 2026-06-17)
...
### Phase 14: Reviews + sentiment — ✅ COMPLETE + LIVE on prod 2026-06-17, 4/4 plans (3 build + 14-04 activation)
...
Plans (3-plan split): **14-01** per-review v4 reviews.list ingest → review_items ✅ LOOP CLOSED 2026-06-16 (build-local, ZERO prod) · **14-02** reply publish-to-Google (v4 updateReply plumbing + unscheduled cron, consent decision RECORDED, legal-sign-off-gated) ✅ LOOP CLOSED 2026-06-16 (build-local, ZERO live publish) · **14-03** LLM sentiment (review_sentiment table + Haiku structured-output classifier + classify-on-ingest) ✅ LOOP CLOSED 2026-06-17 (build-local, ZERO surface). **Phase 14 ✅ COMPLETE + LIVE on prod 2026-06-17, 4/4 plans (14-04 LOOP CLOSED — 385 Wallace reviews).** Prod activation of all three = the Phase-14 gate batch sharing Phase-13 Gate A/B. **14-04** prod-activation gate batch — ✅ LOOP CLOSED 2026-06-17 (`14-04-SUMMARY.md`; 4 ACs PASS, AC-3 "no new secret" sub-clause falsified by deviation #1; a DELTA over 13-04). Scope = PURE activation: the 3 committed Phase-14 migrations (review_items widen+UNIQUE, review_responses publish-state, review_sentiment table+RLS) applied under PROTOCOL + `vercel --prod` the committed gbp-reviews-sync cron (10 crons; the reply cron stays out) + the D1 v4 LIST live-smoke (Google-gated on the shared Gate A/B + the legacy-GMB-API 300 QPM line) + the D2 gateway-Haiku sentiment smoke (NOT Google-gated; asserts review_sentiment ROWS). Partial-now/pending-close; Closed LIVE (Gate A/B had cleared since 13-04): 3 migrations applied under PROTOCOL (advisor 124→124 security-clean), gbp-reviews-sync cron deployed (10 crons), Wallace re-consented under business.manage → 385 real reviews ingested, sentiment classifier proven live. Two fix-forward deviations: GBP OAuth re-pointed to the operator's separate n8n-workspace-apis client (gbpOAuthClientEnv + 13-file thread + 3 new env vars; supersedes 13-04's "no new secret") + a Wallace public.locations backfill (NOT-NULL FK, systemic fleet gap). Open tails: rotate the chat-exposed GBP secret; the D3 7-day token pass-gate; the systemic locations fleet backfill. OUT-of-scope named follow-ups (NOT the gate batch): **14-02b** reply-publish live activation (legal sign-off + consent/authorization schema + UI publish button + the reply cron + v4 WRITE smokes); **14-03b** sentiment surface (report block + dashboard panel + low-confidence human-review queue + a CI golden-set gate).
...
*Last updated: 2026-06-18 — **Phase 15 (Billing foundation + Stripe spine) PLANNED** — 3 build-local plans created (15-01 webhook spine + tier reconcile [W1] · 15-03 PII-at-rest infra [W1, parallel] · 15-02 invoices/payments financial-record-only + handlers [W2 ← 15-01]); all autonomous, ZERO prod, migrations authored-not-applied; financial-record-only mirror + PII-at-rest-as-infra (operator-confirmed); a separate prod gate plan is authored after the build-local plans close. Research gate satisfied (phase-15 RESEARCH, today; defects confirmed by file reads). Next: /paul:apply (Wave 1 = 15-01 + 15-03 parallel). ── prior 2026-06-17: **v0.3.5 Presence + Sentiment ✅ COMPLETE + CLOSED 2026-06-17** (2 phases / 10 plans; GBP presence + reviews/sentiment LIVE on prod, 385 Wallace reviews; tag `v0.3.5`; archived). Next: /paul:discuss-milestone (v0.4 Invoicing + Payments queued). ── prior: 14-04 ✅ LOOP CLOSED 2026-06-17: Phase 14 LIVE on prod (385 Wallace reviews; 3 migrations; gbp-reviews-sync cron deployed; 2 fix-forward deviations — GBP OAuth re-point + locations backfill). ── prior: Phase 14 (reviews read/reply + LLM sentiment) ✅ COMPLETE 2026-06-17 (activation-pending) — the reviews vertical built build-local (14-01 per-review v4 ingest + 14-02 reply publish-to-Google plumbing [consent-gated] + 14-03 LLM sentiment [review_sentiment + Haiku classify-on-ingest]); prod activation = the Phase-14 gate batch sharing Phase-13 Gate A/B. Phase 13 (GBP) ✅ COMPLETE 2026-06-16. Next: /paul:complete-milestone v0.3.5. ── prior 2026-06-13: milestone v0.3 ✅ COMPLETE + CLOSED; v0.3.5 created.*
