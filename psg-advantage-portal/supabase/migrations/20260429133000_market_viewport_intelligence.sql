...
AS $$
  ...
  visible_zips AS (
    SELECT DISTINCT zb.zip_code AS zip
    FROM zipcode_boundaries zb, bounds
    WHERE zb.boundary IS NOT NULL
      AND ST_Intersects(zb.boundary, bounds.geom)
  ),
  visible_points AS (
    SELECT points.*
  ...
  crash_by_zip AS (
    ...
    JOIN visible_zips vz ON vz.zip = cza.zip
  ...
  storm_by_zip AS (
    ...
    JOIN visible_zips vz ON vz.zip = szm.zip
  ...
  zip_rollup AS (
    SELECT
      vz.zip,
      COALESCE(cbz.state, 'US') AS state,
      cbz.city,
      COALESCE(cbz.year, (SELECT year FROM latest_crash_year)) AS year,
    ...
    FROM visible_zips vz
    LEFT JOIN crash_by_zip cbz ON cbz.zip = vz.zip
    LEFT JOIN storm_by_zip sbz ON sbz.zip = vz.zip AND sbz.year = cbz.year
  ),
  totals AS (
    SELECT
